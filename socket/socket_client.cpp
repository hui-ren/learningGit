...
如果ws2_32.dll不可用,你的计算机连接网络的操作会不稳定(即使不连接到外部网络)。*/
	...
	addr.sin_port = htons(1234); //converts a u_short from host to TCP/IP network byte order (which is big-endian)
	...
	//5.send && receive
	char buf[128] = "You're so remarkable! \n";
	int res = send(hclient, buf, strlen(buf),0);
	if (res > 0) { printf("发送的消息是：%s",buf); } 
	...
	*/
	...
	if (rec > 0) { printf("发送的消息是：%s", buf_recv); }
